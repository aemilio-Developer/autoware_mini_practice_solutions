...
class LocalPathExtractor:
   ...
   def extract_local_path(self, _):
       try:
           ...
           """
           Steps performed:
                1. Extracts the XY coordinates from the 3D global path.

                2. Computes the differences (deltas) between consecutive points.

                3. Calculates Euclidean distance (segment_lengths) for each segment.

                4. Uses np.cumsum() to get cumulative distances.

                5. Prepends 0.0 to make the first point's distance zero.
           """ 
